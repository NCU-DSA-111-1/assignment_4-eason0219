...
int count=0;
struct node{
    int snake[2][SNAKE_ARRAY_SIZE],length,foodx,foody,dir,speed,waitime,score;
...
void replaymove(int snake[][SNAKE_ARRAY_SIZE], int Length, int dir)
{
    int i; 
    int x;
    int y;
    ...
    gotoxy(x,y);
    printf(" ");
    gotoxy(snake[0][0],snake[1][0]);
    printf("\033[33m■\033[m");
    snake[0][0]=current->next->snake[0][0];
    snake[1][0]=current->next->snake[1][0];
...
void savegame(){
    ...
    {
        for(j=0;j<2;j++){
            current->snake[j][i]=snakeXY1[j][i];
        }
    }
...
void replay(){
    ...
    while (current->next!=NULL){
        ...
        if(clock() >= endWait){ //it moves according to how fast the computer running it is...
            replaymove(current->snake, current->length, current->dir);
