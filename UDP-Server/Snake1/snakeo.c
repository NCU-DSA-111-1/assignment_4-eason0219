...
int checkKeysPressed1(int direction1)
{
    int pressed1;
...
char checkKeysPressed2(char direction2)
{
    char pressed2;
    //If a key has been pressed
    if(kbhit()){
...
int generateFood1(int foodXY1[], int width, int height, int snakeXY1[][SNAKE_ARRAY_SIZE], int snakeLength1)
{
    do{
        srand ( time(NULL) );
        foodXY1[0] = rand() % (width-3) + 2;
        if(foodXY1[0]%2==1)
            foodXY1[0]++;
        srand ( time(NULL) );
        foodXY1[1] = rand() % (height-6) + 2;
...
int generateFood2(int foodXY2[], int width, int height, int snakeXY2[][SNAKE_ARRAY_SIZE], int snakeLength2)
{
    do{
        srand ( time(NULL) );
        foodXY2[0] = rand() % (width-3) + 2;
        if(foodXY2[0]%2==1)
            foodXY2[0]++;
        srand ( time(NULL) );
        foodXY2[1] = rand() % (height-6) + 2;
...
int collisionDetection1(int snakeXY1[][SNAKE_ARRAY_SIZE], int consoleWidth, int consoleHeight, int snakeLength1 ) //Need to Clean this up a bit
{
    int colision1 = 0;
    if ((snakeXY1[0][0] == 1) || (snakeXY1[0][0] + 1 == 1) || (snakeXY1[1][0] == 1) || (snakeXY1[0][0] == consoleWidth) || (snakeXY1[1][0] == consoleHeight - 4)) //Checks if the snake collided wit the wall or it's self
        ...
        if (collisionSnake(snakeXY1[0][0], snakeXY1[1][0], snakeXY1, snakeLength1, 1)) //If the snake collided with the wall, there's no point in checking if it collided with itself.
            colision1 = 1;
    return colision1;
}

int collisionDetection2(int snakeXY2[][SNAKE_ARRAY_SIZE], int consoleWidth, int consoleHeight, int snakeLength2) //Need to Clean this up a bit
{
    int colision2 = 0;
    if ((snakeXY2[0][0] == 1) || (snakeXY2[0][0] + 1 == 1) || (snakeXY2[1][0] == 1) || (snakeXY2[0][0] == consoleWidth) || (snakeXY2[1][0] == consoleHeight - 4)) //Checks if the snake collided wit the wall or it's self
        ...
        if (collisionSnake(snakeXY2[0][0], snakeXY2[1][0], snakeXY2, snakeLength2, 1)) //If the snake collided with the wall, there's no point in checking if it collided with itself.
            colision2 = 1;
    return colision2;
}

void refreshInfoBar1(int score1, int speed1)
{
    gotoxy(5,37);
...
void refreshInfoBar2(int score2, int speed2)
{
    gotoxy(5,38);
...
void startGame1( int snakeXY1[][SNAKE_ARRAY_SIZE], int foodXY1[], int consoleWidth, int consoleHeight, int snakeLength1, int direction1, int score1, int speed1)
{
    int gameOver1 = 0;
    clock_t endWait1;
    ...
    do{
        if(canChangeDirection1){
            ...
            direction1 = checkKeysPressed1(direction1);
        }
        ...
        if(clock() >= endWait1){ //it moves according to how fast the computer running it is...
            ...
            if(eatFood1(snakeXY1, foodXY1,direction1)){
                generateFood1( foodXY1, consoleWidth, consoleHeight, snakeXY1, snakeLength1); //Generate More Food
                snakeLength1++;
                score1+=10;
        ...
        gameOver1 = collisionDetection1(snakeXY1, consoleWidth, consoleHeight, snakeLength1);
...
void startGame2( int snakeXY2[][SNAKE_ARRAY_SIZE], int foodXY2[], int consoleWidth, int consoleHeight, int snakeLength2, char direction2, int score2, int speed2)
{
    int gameOver2 = 0;
    clock_t endWait2;
    ...
    do{
        if(canChangeDirection2){
            ...
            direction2 = checkKeysPressed2(direction2);
        }
        ...
        if(clock() >= endWait2){ //it moves according to how fast the computer running it is...
            ...
            if(eatFood2(snakeXY2, foodXY2,direction2)){
                generateFood2( foodXY2, consoleWidth, consoleHeight, snakeXY2, snakeLength2); //Generate More Food
                snakeLength2++;
                score2+=10;
        ...
        gameOver2 = collisionDetection2(snakeXY2, consoleWidth, consoleHeight, snakeLength2);
...
void loadGame1()
{
    ...
    int foodXY1[] = {6,6};// Stores the location of the food
    int score1 = 0;
    ...
    generateFood1( foodXY1, consoleWidth, consoleHeight, snakeXY1, snakeLength1);
    refreshInfoBar1(score1, speed1); //Bottom info bar. Score, Level etc
    startGame1(snakeXY1, foodXY1, consoleWidth, consoleHeight, snakeLength1, direction1, score1, speed1);
    return;
    // pthread_exit(NULL);
}
...
void *loadGame2()
{
    ...
    int foodXY2[] = {5,5};// Stores the location of the food
    int score2 = 0;
    int consoleWidth = 150;
    int consoleHeight = 35;
    int speed2 = 2;
    ...
    generateFood2( foodXY2, consoleWidth, consoleHeight, snakeXY2, snakeLength2);
    refreshInfoBar2(score2, speed2); //Bottom info bar. Score, Level etc
    startGame2(snakeXY2, foodXY2, consoleWidth, consoleHeight, snakeLength2, direction2, score2, speed2);
    pthread_exit(NULL);
}
