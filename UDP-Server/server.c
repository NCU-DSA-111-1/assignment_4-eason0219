...
void addNewClient(char* ip,unsigned short port,char* name);
void freestruct();
...
void *clientmessage(void* arg){
    ...
    while(1){
        ...
        if(recv_ret > 0){
            ...
            switch(cmsg.FLAG){
                case LOGIN:
                    addNewClient(ip,port,cmsg.client_name);
                    break;
                case QUIT:
...
void deleteClient(int deletenum){
    ...
    printf("\nClient %d Logout\n- Name: %s\n- Port:%hu\n- IP:%s\n",deleteptr->next->client_num,deleteptr->next->client_name,deleteptr->next->client_port,deleteptr->next->client_ip);
...
void sendmessage(cmessage *cmsg){
    ...
    while(msgptr->next!=NULL){
        ...
        if(send_ret == -1)
            ...
            printf("Send to client %d port: %hu IP: %s\n",msgptr->client_num,msgptr->client_port,msgptr->client_ip);
    }
}
...
void addNewClient(char* ip,unsigned short port,char* name){
    ...
    strcpy(new->client_name,name);
    new->client_port=port;
    ClientData->next=new;
    new->next=NULL;
    ClientData=ClientData->next;
    // printf("New Client Login\n- Name: %s\n",ClientData->client_name);
    printf("\nNew Client Login\n- Name: %s\n- IP:%s\n- Port:%hu\n",ClientData->client_name,ClientData->client_ip,ClientData->client_port);
    return;
}
void main(){
    int bind_ret;
